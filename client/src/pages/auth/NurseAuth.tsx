...
import { ArrowRight, Loader2, Mail, Activity, UserX } from "lucide-react";
import { Suspense, useEffect, useState } from "react";
import { useNavigate } from "react-router-dom";
import { motion } from "framer-motion";

export default function NurseAuth() {
  ...
  return (
    <div className="min-h-screen flex flex-col bg-gradient-to-br from-triage-blue/5 via-background to-background relative">
      ...
      <div className="flex-1 flex items-center justify-center p-6 relative z-10">
        <div className="flex flex-col items-center gap-6">
          <div className="flex items-center gap-3 mb-4">
            <div className="p-3 bg-triage-blue/10 rounded-lg">
              <Activity className="w-8 h-8 text-triage-blue" />
            </div>
            <div>
              <h1 className="text-2xl font-bold tracking-tight">Nurse Portal</h1>
              <p className="text-sm text-muted-foreground font-mono">TRIAGE MANAGEMENT</p>
            </div>
          </div>

          <Card className="w-full max-w-lg pb-0 border shadow-md border-triage-blue/20">
            <CardHeader className="text-center">
              <CardTitle className="text-xl">Nurse Access</CardTitle>
              <CardDescription>
            ...
            <form onSubmit={handleSubmit}>
              <CardContent className="space-y-4">
                <div className="p-3 bg-muted/50 rounded-lg border border-border">
                  ...
                  <p className="text-xs font-mono">nurse@cityhospital.com / nurse123</p>
                </div>
                <div className="relative">
                  <Mail className="absolute left-3 top-3 h-4 w-4 text-muted-foreground" />
                  <Input
                    placeholder="nurse@cityhospital.com"
                    type="email"
                    className="pl-9"
                    value={email}
